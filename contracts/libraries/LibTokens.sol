...
library LibTokens {
    ...
    // function _balanceOfERC20Token(address _token)
    //     internal
    //     view
    //     returns (uint256 balance_)
    // {
    //     balance_ = IERC20(_token).balanceOf(address(this));
    // }

    // function _allowanceOfERC20Token(address _token,address _spender) internal view returns(uint256 allowance_){
    //     allowance_=IERC20(_token).allowance(address(this),_spender);
    // }
...
function _safeInputERC1155Token(address _token,uint256 _tokenID,uint256 _value,bytes calldata _data ) internal {
    Guards._notExpired();
    IERC1155(_token).safeTransferFrom(msg.sender,address(this),_tokenID,_value,_data);
   }

   function _safeBatchInputERC1155Tokens(address token,uint256[] calldata _tokenIDs,uint256[] calldata _values,bytes calldata _data) internal {
    Guards._notExpired();
    IERC1155(token).safeBatchTransferFrom(msg.sender,address(this),_tokenIDs,_values,_data);
   }
