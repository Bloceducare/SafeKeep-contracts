...
contract ERC721Facet {
    ...
    returns (AllocatedERC721Tokens[] memory allocated)
  {
    ...
    if (tokenAddressCount > 0) {
      ...
      for (uint256 i; i < tokenAddressCount; i++) {
        address _token = vaultData.inheritorAllocatedERC721TokenAddresses[
          _inheritor
        ][i];
  ...
  function getAllocatedERC721TokenIds(address _inheritor, address _token)
    external
    view
  ...
  function getAllocatedERC721TokenAddresses(address _inheritor)
    public
    view
  ...
  function depositERC721Tokens(address _token, uint256[] calldata _tokenIDs)
    external
  {
