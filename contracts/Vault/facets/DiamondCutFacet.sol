...
contract DiamondCutFacet is IDiamondCut {
    ...
    function diamondCut(
        ...
        if (msg.sender != IVaultDiamond(address(this)).vaultFactoryDiamond())
            revert LibErrors.NoPermissions();
        LibDiamond.diamondCut(_diamondCut, _init, _calldata);
    }

    //temp call made from factory to confirm ownership
    // function tempOwner() public view returns (address owner_) {
    //     DMS storage vaultData = LibStorageBinder._bindAndReturnVaultStorage();
