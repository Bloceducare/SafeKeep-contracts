...
library LibKeep {
    ...
    returns (uint256 alloc_)
  {
    VaultStorage storage vs = LibDiamond.vaultStorage();
    for (uint256 x; x < vs.inheritors.length; x++) {
      alloc_ += vs.inheritorERC1155TokenAllocations[vs.inheritors[x]][_token][
        _tokenID
      ];
    }
  }
  ...
  function _allocateEther(
    ...
    VaultStorage storage vs = LibDiamond.vaultStorage();
    for (uint256 k; k < _inheritors.length; k++) {
      if (!Guards._activeInheritor(_inheritors[k])) revert InactiveInheritor();
      //update storage
  ...
  function _allocateERC721Tokens(
    ...
    if (_inheritors.length == 0 || _tokenIDs.length == 0) revert EmptyArray();
      ...
      if (IERC721(_token).ownerOf(_tokenIDs[k]) == address(this)) {
        if (vs.allocatedERC721Tokens[_token][_tokenIDs[k]]) {
          ...
          if (current != _inheritors[k] && current != address(0)) {
            //    vs.inheritorERC721Tokens[current][_token] = 0;
            // type(uint256).max -
            // 2;
          ...
          if (_inheritors[k] == address(0)) {
            ...
            LibKeepHelpers.removeUint(
              vs.inheritorAllocatedTokenIds[_inheritors[k]][_token],
              _tokenIDs[k]
            );
            if (
              vs.inheritorAllocatedTokenIds[_inheritors[k]][_token].length == 0
            ) {
              LibKeepHelpers.removeAddress(
                vs.inheritorAllocatedERC721TokenAddresses[_inheritors[k]],
                _token
              );
            }
          }
        ...
        if (vs.inheritorAllocatedTokenIds[_inheritors[k]][_token].length == 0) {
          vs.inheritorAllocatedERC721TokenAddresses[_inheritors[k]].push(
            _token
          );
        }
  ...
  function _allocateERC1155Tokens(
    ...
    for (uint256 i; i < _inheritors.length; i++) {
      ...
      if (!Guards._activeInheritor(_inheritors[i])) revert InactiveInheritor();
  ...
  function _withdrawERC20Token(
    ...
    if (currentBalance >= availableTokens) {
      ...
      try IERC20(_token).transfer(_to, _amount) {} catch {
        //assumes all ERC20 errors have a revert string
        string memory reason;
        if (bytes(reason).length == 0) {
  ...
  function _withdrawERC1155Token(
    ...
    if (balance < _amount) revert InsufficientTokens();

    if (balance - allocated < _amount) revert("UnAllocate TokensFirst");
  ...
  function _claimOwnership(address _newBackup) internal {
    ...
    address prevOwner = vs.vaultOwner;
    address prevBackup = vs.backupAddress;
    assert(_newBackup != msg.sender && prevOwner != _newBackup);
      ...
      for (uint256 i; i < tokens; i++) {
        ...
        if (noOfTokenIds > 0) {
          for (uint256 k; k < noOfTokenIds; k++) {
            ...
            uint256 amount = vs.inheritorERC1155TokenAllocations[msg.sender][
              token
            ][tokenID];
            if (amount > 0) {
              vs.inheritorERC1155TokenAllocations[msg.sender][token][
                tokenID
              ] = 0;
