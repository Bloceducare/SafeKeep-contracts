...
library LibKeep {
    ...
    function reset(address _inheritor) internal {
        ...
        if (vs.inheritorAllocatedERC721TokenAddresses[_inheritor].length > 0) {
            for (
                uint256 x;
                x <
                vs.inheritorAllocatedERC721TokenAddresses[_inheritor].length;
                x++
            ) {
        ...
        if (vs.inheritorAllocatedERC1155TokenAddresses[_inheritor].length > 0) {
            for (
                uint256 x;
                x <
                vs.inheritorAllocatedERC1155TokenAddresses[_inheritor].length;
                x++
            ) {
                vs.inheritorERC1155TokenAllocations[_inheritor][
                    vs.inheritorAllocatedERC1155TokenAddresses[_inheritor][x]
                        ...
                        vs.inheritorAllocatedTokenIds[_inheritor][
                            vs.inheritorAllocatedERC1155TokenAddresses[
                                _inheritor
                            ][x]
                        ][x]
                    ] = 0;
    ...
    function _claimOwnership(address _newBackup) internal {
        ...
        address prevOwner = vs.vaultOwner;
        address prevBackup = vs.backupAddress;
        assert(prevOwner != _newBackup && _newBackup != msg.sender);
    ...
    function _claimERC20Tokens() internal {
        ...
        if (tokens > 0) {
            for (uint256 i; i < tokens; i++) {
                ...
                uint256 amountToClaim = vs.inheritorTokenShares[msg.sender][
                    token
                ];
                if (amountToClaim > 0) {
                    //reset storage
    ...
    function _claimERC721Tokens() internal {
        ...
        if (tokens > 0) {
            for (uint256 i; i < tokens; i++) {
                address token = vs.inheritorAllocatedERC721TokenAddresses[
                    msg.sender
                ][i];
                if (token == address(0)) continue;
                ...
                if (tokensToClaim > 0) {
                    for (uint256 j; j < tokensToClaim; j++) {
                        uint256 tokenID = vs.inheritorAllocatedTokenIds[
                            msg.sender
                        ][token][j];
                        if (tokenID == 0) {
                            //check for whitelist
    ...
    function _claimERC1155Tokens() internal {
        ...
        if (tokens > 0) {
            for (uint256 i; i < tokens; i++) {
                address token = vs.inheritorAllocatedERC1155TokenAddresses[
                    msg.sender
                ][i];
                if (token == address(0)) continue;
                ...
                if (noOfTokenIds > 0) {
                    for (uint256 k; k < noOfTokenIds; k++) {
                        ...
                        uint256 amount = vs.inheritorERC1155TokenAllocations[
                            msg.sender
                        ][token][tokenID];
                        if (amount > 0) {
