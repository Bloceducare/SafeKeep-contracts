...
library LibTokens {
            ...
            try IERC20(token).transferFrom(msg.sender, address(this), amount) {
                ...
                if (success) {
                    emit ERC20TokenDeposit(
                        token, msg.sender, amount, LibKeep._vaultID()
                        );
                } else {
    ...
    function _inputERC721Token(address _token, uint256 _tokenID) internal {
        ...
        emit ERC721TokenDeposit(
            _token, msg.sender, _tokenID, LibKeep._vaultID()
            );
    }
        ...
        internal
    {
        ...
        IERC721(_token).safeTransferFrom(
            msg.sender, address(this), _tokenID, _data
        );
    }

    function _approveERC721Token(address _token, uint256 _tokenID, address _to)
        internal
    {
        Guards._notExpired();
    ...
    function _approveAllERC721Token(address _token, address _to, bool _approved)
        internal
    {
        Guards._notExpired();
        ...
        internal
    {
        ...
        IERC1155(_token).safeTransferFrom(
            msg.sender, address(this), _tokenID, _value, ""
        );
        emit ERC1155TokenDeposit(
            _token, msg.sender, _tokenID, _value, LibKeep._vaultID()
            );
    }
        ...
        internal
    {
        ...
        IERC1155(_token).safeBatchTransferFrom(
            msg.sender, address(this), _tokenIDs, _values, ""
        );
        emit BatchERC1155TokenDeposit(
            _token, msg.sender, _tokenIDs, _values, LibKeep._vaultID()
            );
    }
