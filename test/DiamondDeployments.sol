...
contract DDeployments is Test, IDiamondCut {
  ...
  function setUp() public {
    ...
    Vault1Owner = mkaddr("vault1");

    address s = mkaddr("ann");
    vm.startPrank(Vault1Owner, Vault1Owner);
    vm.deal(Vault1Owner, 1 ether);
    ...
    Vault2 = VaultSpawnerFacet(address(vFactoryDiamond)).createVault{
      value: 2 ether
    }(toSingletonAdd(s), toSingletonUINT(10000), 2e18, mkaddr("Vault2 BackUp"));
    vm.stopPrank();
  }
